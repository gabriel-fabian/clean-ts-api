...
describe('DbLoadSurveyResult UseCase', () => {
  ...
  test('Call LoadSurveyResultRepository with correct values', async () => {
  ...
  test('Throw if LoadSurveyResultRepository throws', async () => {
    const { sut, loadSurveyResultRepositoryStub } = makeSut()
  ...
  test('Call LoadSurveyByIdRepository if LoadSurveyResultRepository returns null', async () => {
  ...
  test('Return surveyResultModel with all answers with count 0 if LoadSurveyResult returns null', async () => {
    const { sut, loadSurveyResultRepositoryStub } = makeSut()
    jest.spyOn(loadSurveyResultRepositoryStub, 'loadBySurveyId').mockResolvedValueOnce(null)
  ...
  test('Return surveyResultModel on success', async () => {
    const { sut } = makeSut()
    const surveyResult = await sut.load('any_survey_id', 'any_account_id')
    expect(surveyResult).toEqual(mockSurveyResultModel())
  })
